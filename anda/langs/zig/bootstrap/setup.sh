...
if [ "$1" == "fetch" ]; then
   ...
   curl -A "rpmdev-spectool" -H "Accept-Encoding: identity" ${mirror}/zig-${version}.tar.xz?source=terra.fyralabs.com
   curl -A "rpmdev-spectool" -H "Accept-Encoding: identity" ${mirror}/zig-${version}.tar.xz.minisig?source=terra.fyralabs.com
elif [ "$1" == "version" ]; then
   echo $version
