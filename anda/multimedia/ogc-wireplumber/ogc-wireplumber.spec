%global _name wireplumber
Name:       ogc-wireplumber
Version:    0.5.13
Release:    1%{?dist}
...
Source0:    https://gitlab.freedesktop.org/pipewire/%{_name}/-/archive/%{version}/%{_name}-%{version}.tar.bz2
...
## OGC patches
Patch:         https://raw.githubusercontent.com/OpenGamingCollective/wireplumber/refs/tags/%{version}/block_steam_clear_default.patch
...
Recommends:     %{name}%{?_isa} = %{version}-%{release}
Provides:       %{_name}-libs%{?_isa} = %{version}-%{release}
...
Requires:       %{name}-libs%{?_isa} = %{version}-%{release}
Provides:       %{_name}-devel%{?_isa} = %{version}-%{release}
...
Recommends:     %{name}%{?_isa} = %{version}-%{release}
Provides:       %{_name}-doc%{?_isa} = %{version}-%{release}
...
%posttrans
%systemd_user_post %{_name}.service

%preun
%systemd_user_preun %{_name}.service

%triggerun -- fedora-release < 35
# When upgrading to Fedora Linux 35, transition to WirePlumber by default
if [ -x "/bin/systemctl" ]; then
    /bin/systemctl --no-reload preset --global %{_name}.service || :
fi
