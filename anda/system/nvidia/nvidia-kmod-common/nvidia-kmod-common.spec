...
if lspci | grep -i NVIDIA | head -n 1 | cut -d ':' -f 3 | cut -d '[' -f 1 | sed -e 's/^[[:space:]]*//' | grep "10de:$id" &>/dev/null;
 then
  if cat %{_sysconfdir}/nvidia/kernel.conf | grep "MODULE_VARIANT=kernel-open" &>/dev/null;
    then sed -i -e 's/kernel-open$/kernel/g' %{_sysconfdir}/nvidia/kernel.conf &>/dev/null
